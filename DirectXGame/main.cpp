...
int WINAPI WinMain(_In_ HINSTANCE, _In_opt_ HINSTANCE, _In_ LPSTR, _In_ int) {
	...
	device->CreateRenderTargetView(
	    ...
	    nullptr,               // RTVの詳細情報(Desc:Description、構成内容の記述)
	                           // ※RTVの場合nullptrにするとDirectX12が自動で推奨してくれる
	...
	//
	// 0. DepthStencilTextureResouceの作成
	...
	device->CreateShaderResourceView(
	    ...
	    srvHandleCPU           // SRV用ディスクリプタヒープのCPUHandle
	);
	...
	while (true) {
		...
		barrier.Flags = D3D12_RESOURCE_BARRIER_FLAG_NONE;                            // フラグを None にしておく
